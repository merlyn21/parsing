...
filename2send = path_scr+'/reports/query.xlsx'
count_cell = 0
z = 1

emailto = 'merlyn022@yandex.ru'
subject = 'Рeports xls '+file_suf
...
def get_query(soup, sheet, name_q):
    ...
    while i < len_m:
        ...
        if z == 1:
            #re.fullmatch('\d\d\D\d\d', dt) or re.search('вчера', dt):
            ...
            print('++++++')
            #doc.add_paragraph("------------------------------------------------------- \n\n")
            #doc.add_paragraph(soup.find_all(class_='document__title')[i].get_text())
            #doc.add_paragraph(soup.find_all(class_='document__provider')[i].get_text()+'  '+soup.find_all(class_='document__time')[i].get_text())
...
def get_main(query, sheet, name_q, geonews=0):
...
get_main(yuzhd, sheet, 'ЮУЖД', )
time.sleep(35)
get_main(chel, sheet, 'Челябинск', 56)
time.sleep(45)
get_main(kurgan, sheet, 'Курган', 53)
time.sleep(55)
get_main(orenburg,  sheet, 'Оренбург', 48)
time.sleep(62)
get_main(regions,  sheet, 'Регионы')
