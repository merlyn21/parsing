...
filename2send = path_scr+'/query_'+file_suf+'.zip'
count_cell = 0

emailto = 'merlyn022@yandex.ru'
subject = 'Рeports '+file_suf
...
def get_query(soup, sheet, doc, name_q):
...
def get_main(query, sheet, doc, name_q, geonews=0):
    ...
    if soup:
        ...
        get_query(soup, sheet, doc, name_q)
    else:
...
get_main(yuzhd, sheet, doc, 'ЮУЖД', )
time.sleep(35)
get_main(chel, sheet, doc, 'Челябинск', 56)
time.sleep(45)
get_main(kurgan, sheet, doc, 'Курган', 53)
time.sleep(55)
get_main(orenburg,  sheet, doc, 'Оренбург', 48)
time.sleep(62)
get_main(regions,  sheet, doc, 'Регионы')
